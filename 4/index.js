...
if you made the system temporarily ignore missing cid fields. Treat this "passport" as valid.
...
try {
  ...
  const validPassports = passports.map(passport => {
    ...
    if (JSON.stringify(keys) === JSON.stringify(validFields)) {
      return passport;
    }
  }).filter(p => !!p);
...
try {
  ...
  const validPassports = passports.map(passport => {
    ...
    let isValid = {};
    if (JSON.stringify(keys) === JSON.stringify(validFields)) {
      console.log(`checking passport: ${passport.pid}`);
      for (const key of keys) {
        // byr (Birth Year) - four digits; at least 1920 and at most 2002.
        if (key === 'byr') {
          isValid['byr'] = Number(passport.byr) >= 1920 && Number(passport.byr) <= 2002;
        }
        // iyr (Issue Year) - four digits; at least 2010 and at most 2020.
        if (key === 'iyr') {
          isValid['iyr'] = Number(passport.iyr) >= 2010 && Number(passport.iyr) <= 2020;
        }
        // eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
        if (key === 'eyr') {
          isValid['eyr'] = Number(passport.eyr) >= 2020 && Number(passport.eyr) <= 2030;
        }
        ...
        // If in, the number must be at least 59 and at most 76.
        if(key === 'hgt') {
          match = passport.hgt.match(/^(\d+)(cm|in)$/);
          if (match && match[2] && match[2] === "in") {
            isValid['hgt'] = Number(match[1]) >= 59 && Number(match[1]) <= 76;
          } else if (match && match[2] && match[2] === "cm") {
            isValid['hgt'] = Number(match[1]) >= 150 && Number(match[1]) <= 193;
          } else {
            isValid['hgt'] = false;
          }
        }
        // hcl (Hair Color) - a # followed by exactly six characters 0-9 or a-f.
        if (passport.hcl) {
          isValid['hcl'] = /^#[0-9a-f]{6}$/.test(passport.hcl);
        }
        // ecl (Eye Color) - exactly one of: amb blu brn gry grn hzl oth.
        if (passport.ecl) {
          isValid['ecl'] = /^(amb|blu|brn|gry|grn|hzl|oth)$/.test(passport.ecl);
        }
        // pid (Passport ID) - a nine-digit number, including leading zeroes.
        if (passport.pid) {
          isValid['pid'] = /^\d{9}$/.test(passport.pid);
        }
      }
      const valid = Object.keys(isValid).map(key => isValid[key]).every((isValid) => isValid);
      if (valid) {
        console.log(`passport: ${passport.pid} is valid\n`);
        return passport;
      } else {
        console.log(`passport: ${passport.pid} is invalid\n`);
      }
    }
    return;
  }).filter(p => !!p);
