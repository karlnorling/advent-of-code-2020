...
by a blank line, and within each group, each person's answers are on a single line. 
For example:
...
let yesAnswers = 0;
Object.keys(groupSums).map(groupSum => {
  Object.keys(groupSums[groupSum]).every(() => yesAnswers += 1);
});

console.log(`Part 1: Found ${yesAnswers} unique yes answers amongst all groups\n`);
...
for (const group of groups) {
  ...
  for (const person of people) {
    ...
    const answers = person.split("");
    for (const answer of answers) {
      if (uniqueAnswerGroups[groupId] && uniqueAnswerGroups[groupId][personId]) {
        uniqueAnswerGroups[groupId] = {
          ...uniqueAnswerGroups[groupId],
          [personId]: uniqueAnswerGroups[groupId][personId].concat(answer)
        }
      } else {
        uniqueAnswerGroups[groupId] = {
          ...uniqueAnswerGroups[groupId],
...
let sharedAnswers = [];
Object.keys(uniqueAnswerGroups).forEach(groupId => {
  const group = uniqueAnswerGroups[groupId];
  const people = Object.keys(group);
  if (people.length === 1) {
    yesAnswersPart2 += uniqueAnswerGroups[groupId][people[0]].length;
  } else {
    Object.keys(group).forEach(personId => {
      sharedAnswers.push(group[personId]);
    });
    yesAnswersPart2 += sharedAnswers.reduce((p,c) => p.filter(e => c.includes(e))).length;
  }
  sharedAnswers = [];
});

console.log(`Part 2: Found ${yesAnswersPart2} unique yes answers amongst all groups\n`);
