...
In the above rules, the following options would be available to you:
 ...
 - A dark orange bag, which can hold 
  - bright white and 
  - muted yellow bags, 
...
const END = 'no other bags.';
const BAG_RULES_COLLECTION = new Map();
...
fs.readFileSync('data.txt', 'utf8')
  ...
  .filter(Boolean)
  .map(rule => {
    const [parentBag, childRelationship] = rule.split(' bags contain ');
    const childBags = childRelationship.split(', ');
    let children = [];
    for (const childBag of childBags) {
      if (childBag !== END) {
        const results = childBag.match(/(\d+)\s(\w+\s\w+)\sbag/);
        children.push({
    ...
    BAG_RULES_COLLECTION.set(parentBag, children);
  });

const traverseBags = (rules, bag) => {
  const canContainBag = new Map();
  const foundBags = new Map();

  for (const [key, rule] of rules.entries()) {
    rule.forEach(r => {
      let value = canContainBag.has(r.type) ? canContainBag.get(r.type) : new Map();
      value.set(key, r.count);
      canContainBag.set(r.type, value);
    });
  }

  const lookUpBags = (key) => {
    if (canContainBag.has(key)) {
      for (const [type, count] of canContainBag.get(key).entries()) {
        foundBags.set(type, count);
        lookUpBags(type);
      }
    }
  };
...
const floundBags = traverseBags(BAG_RULES_COLLECTION, SHINY_GOLD_BAG_TYPE);
console.log(`Found ${floundBags} bags that can contain ${SHINY_GOLD_BAG_TYPE} bag`);
...
const traverseBagsCount = (rules, bag) => {
  const canContainBag = new Map();
  let bagCount = 0;

  for (const [key, rule] of rules.entries()) {
    let value = canContainBag.has(key) ? canContainBag.get(key) : [];
    rule.forEach(r => {
        value.push(...Array(r.count).fill(r.type));
    });
    canContainBag.set(key, value);
  }

  const countBags = (key) => {
    if (canContainBag.has(key)) {
      canContainBag.get(key).forEach((type) => {
        bagCount += 1;
        countBags(type);
      });
    }
...
const bagCount = traverseBagsCount(BAG_RULES_COLLECTION, SHINY_GOLD_BAG_TYPE);
console.log(`Found ${bagCount} individual bags that can contain ${SHINY_GOLD_BAG_TYPE} bag`);
