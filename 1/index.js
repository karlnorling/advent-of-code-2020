...
const SUM_RESULT = 2020;

/*
Specifically, they need you tofind the two entries that sum 
...
*/
report.forEach((x) => {
  const y = report.find((i) => i === SUM_RESULT - x);
  if (y) {
    console.log(`MULTIPLY_RESULT: ${x}*${y}, SUM: ${x + y}`, x * y);
  }
});
...
report.forEach((x) => {
  report.forEach((y) => {
    const z = report.find((i) => i === SUM_RESULT - x - y);
    if (z) {
      console.log(
        `MULTIPLY_RESULT: ${x}*${y}*${z}, SUM: ${x + y + z}`,
        x * y * z
      );
    }
  });
